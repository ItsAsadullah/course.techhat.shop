    ...
    <div className="space-y-6 w-full">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Manage computer labs, capacity, and status.</p>
        </div>
            ...
            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {labs?.map((lab: TrainingLab) => (
                  <tr key={lab.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-900/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <p className="font-semibold text-slate-900 dark:text-slate-50">{lab.name}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 uppercase">{lab.code}</p>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {lab.room || "—"}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-[11px] font-bold uppercase tracking-wide ${
                        lab.status === "active" ? "bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-400" :
                        lab.status === "maintenance" ? "bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-400" :

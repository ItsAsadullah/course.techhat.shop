    ...
    <div className="space-y-6 max-w-7xl">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Manage batch configuration, capacity, and enrolled students.</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                <Activity className="h-4 w-4 text-slate-500" /> Batch Capacity
              </h3>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="text-xl font-bold text-slate-900">{capacity.effective_shift_capacity} seats</p>
                  </div>
                  <div>
                    <p className="text-sm text-slate-500 text-right">Seat Limit</p>
                ...
                <div className="pt-4 border-t border-slate-100">
                  <div className="flex justify-between items-end mb-2">
                    <p className="text-sm font-medium text-slate-700">Effective Capacity</p>
                    <p className="text-2xl font-bold text-slate-900">{capacity.effective_batch_capacity}</p>
                  </div>
                  
                  <div className="w-full bg-slate-100 rounded-full h-2.5 mb-2 overflow-hidden">
                    <div 
                      className={`h-2.5 rounded-full ${capacity.is_full ? 'bg-red-500' : 'bg-blue-600'}`} 
                      style={{ width: `${Math.min(capacity.utilization_percent, 100)}%` }}
                    ></div>
                  </div>
                  
                  <div className="flex justify-between text-xs text-slate-500">
                    <span className="font-medium text-slate-700">{capacity.occupied} Occupied</span>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
             <h3 className="font-semibold text-slate-900 flex items-center gap-2 mb-4">
                <Layers className="h-4 w-4 text-slate-500" /> Assignment
              </h3>
              {batch.shifts && batch.shifts.length > 0 ? (
                 <div className="space-y-3">
                   {batch.shifts.map((shift, idx) => (
                     <div key={shift.id} className={idx > 0 ? "pt-3 border-t border-slate-100" : ""}>
                       <p className="font-medium text-slate-800">{shift.name_en}</p>
                       <p className="text-sm text-slate-600">{shift.start_time.slice(0,5)} - {shift.end_time.slice(0,5)}</p>
                       <div className="flex gap-4 mt-1">
                         <p className="text-xs text-slate-500">
                           Shift Capacity: <span className="font-semibold text-slate-700">{shift.capacity_override ?? shift.lab?.manual_capacity_limit ?? shift.lab?.usable_computers ?? 'N/A'}</span>
                         </p>
                         {shift.lab && (
                           <p className="text-xs text-slate-500">
                ...
                <p className="text-sm text-slate-500 italic">No shifts assigned</p>
              )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="border-b border-slate-100 bg-slate-50 px-6 py-4 flex items-center justify-between">
          <h3 className="font-semibold text-slate-900 flex items-center gap-2">
            <Users className="h-4 w-4 text-slate-500" /> Enrolled Students
          </h3>
          <span className="text-xs font-medium text-slate-500 bg-slate-200 px-2 py-1 rounded-full">{roster?.length || 0}</span>
        </div>
